...
Component({
  ...
  methods: {
    ...
    subscribe({
      ...
      if (res["84qfRZ1dwl4hXz7wtnITSBk_aM8rxPICHPvnYm3xQHE"] === "accept") {
        pActivePush.where({ openid: userInfo.data._openid }).get({
          success: (res) => {
            if (!res.data.length) {
              pActivePush.add({
                data: {
    ...
    onPopupClose() {
      ...
      serviceStore.setState("hIndex", -1);
      this.triggerEvent("close");
    },
    onChangeIndex: function (currentIndex) {
      this.setData({
        currentIndex,
        btnActive: currentIndex > -1,
      });
    },
    onChangeHairstylist(list) {
      this.setData({
        hairstylist: list,
      });
    },
  ...
  lifetimes: {
    ...
    detached() {
      serviceStore.offState("hIndex", this.onChangeIndex);
    },
  },
});
